...
import { files, importAsset, pickFiles, pickFolders, listFolder, pickSavePath, savePdf, htmlToPng } from './platform';
...
/**
 * Save a note as a file, picked in a save panel. Markdown is the note itself; a picture is drawn from
 * the same HTML the editor shows; a PDF is the window printed to that file — paginated, its text still
 * text, and no printer anywhere in it.
 */
...
function page(title: string, body: string): string {
  ...
  .sheet { width: 820px; padding: 72px; box-sizing: border-box; margin: 0 auto; }
