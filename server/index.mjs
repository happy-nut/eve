...
`);
for (const col of ['grp TEXT NOT NULL DEFAULT \'\'', 'ord REAL NOT NULL DEFAULT 0', 'icon TEXT NOT NULL DEFAULT \'\'']) {
...
const qUpsert = db.prepare(`INSERT INTO notes (id, body, updated_at, deleted, grp, ord, icon, seq) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  ON CONFLICT(id) DO UPDATE SET body = excluded.body, updated_at = excluded.updated_at, deleted = excluded.deleted, grp = excluded.grp, ord = excluded.ord, icon = excluded.icon, seq = excluded.seq`);
const qSince = db.prepare('SELECT id, body, updated_at, deleted, grp, ord, icon FROM notes WHERE seq > ? ORDER BY seq');
...
export function handleSync({ cursor = 0, notes = [] }) {
  ...
  try {
    for (const n of notes) {
      ...
      qUpsert.run(n.id, n.body, n.updatedAt, n.deleted ? 1 : 0, typeof n.group === 'string' ? n.group.slice(0, 200) : '', Number.isFinite(n.order) ? n.order : 0, typeof n.icon === 'string' ? n.icon.slice(0, 16) : '', ++seq);
    }
    tx('COMMIT');
  } catch (e) { tx('ROLLBACK'); throw e; }
  const changed = qSince.all(Number(cursor) || 0).map((r) => ({
    id: r.id, body: r.body, updatedAt: r.updated_at, deleted: !!r.deleted, group: r.grp, order: r.ord, icon: r.icon || undefined,
  }));
