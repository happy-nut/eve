...
- **Sidebar** with search (`⌘K`), collapsible **groups** (folders). Drag notes to reorder or move between groups; `⌘\` opens and focuses the list (press again from the list to close it and return to the editor) (↑↓ move, ⌥↑↓ reorder / move across groups, Space or ← → fold, ⌫ deletes with confirmation, Esc back).
